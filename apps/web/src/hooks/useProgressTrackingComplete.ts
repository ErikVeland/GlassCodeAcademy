...
export const useProgressTrackingComplete = () => {
  ...
  const [tierModuleCache, setTierModuleCache] = useState<
    Record<string, string[]>
  >({});
  const [lessonCountCache, setLessonCountCache] = useState<
  ...
  const getModulesByTier = useCallback(
    async (tierSlug: string): Promise<RegistryModule[]> => {
      const reg = await getRegistryCached();
      return reg.modules
  ...
  const getModulesByTrack = useCallback(
    async (track: string): Promise<RegistryModule[]> => {
      const reg = await getRegistryCached();
        ...
        for (const tierSlug of tierSlugs) {
          ...
          for (const modSlug of moduleSlugs) {
            try {
              ...
              lessonCounts[modSlug] = Array.isArray(lessons)
                ? lessons.length
                : lessonCounts[modSlug] || 0;
            } catch {
    ...
    (moduleId: string, lessonsCompleted: number) => {
      ...
      if (streak.lastActiveDate !== today) {
        ...
        const activityEntry = {
          ...
          activity:
            lessonsCompleted > 0
              ? `Completed ${lessonsCompleted} lessons in ${moduleId}`
              : `Accessed ${moduleId}`,
          points: lessonsCompleted > 0 ? lessonsCompleted * 10 : 5,
        };

        updatedStreak.streakHistory = [
        ...
        updatedStreak.milestones = STREAK_MILESTONES.map((milestone) => ({
          ...
          achievedDate:
            updatedStreak.currentStreak >= milestone.threshold &&
            !milestone.achieved
  ...
  const checkAchievements = useCallback(
    async (progressData: Record<string, ProgressData>, moduleId: string) => {
  ...
  const updateUserStats = useCallback(
    (progressData: Record<string, ProgressData>) => {
      ...
      if (firstAccessDates.length > 0) {
        ...
        learningVelocity =
          weeksSinceStart > 0 ? modulesCompleted / weeksSinceStart : 0;
      }
