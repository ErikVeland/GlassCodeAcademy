...
type AchievementStorage = Record<
  string,
  {
    earned: boolean;
    earnedDate?: string;
...
export const useAchievements = () => {
  ...
  useEffect(() => {
    const achievementData = achievements.reduce<AchievementStorage>(
      (acc, achievement) => {
        acc[achievement.id] = {
  ...
  const checkAchievement = useCallback(
    (achievementId: string, progressData: ProgressSnapshot) => {
  ...
  const getAchievementProgress = useCallback(
    (achievementId: string): AchievementProgress[string] | null => {
  ...
  const resetAchievements = useCallback(() => {
    setAchievements(
      Object.values(ENHANCED_ACHIEVEMENTS).map((achievement) => ({
        ...achievement,
        earned: false,
        progress: 0,
      })),
