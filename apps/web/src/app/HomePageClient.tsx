              ...
              <div
                ...
                title={
                  achievement.moduleId === module.slug
                    ...
                    : achievement.tier
                      ? `Tier ${achievement.tier} achievement: ${achievement.description}`
        ...
        >
          ...
          <div className="flex flex-wrap gap-1 mb-4">
            {(module.technologies || []).slice(0, 3).map((tech) => (
              <span
                key={tech}
                className="px-2 py-1 bg-surface-alt text-fg rounded-full text-xs"
              >
                {tech}
              </span>
            ))}
            {(module.technologies || []).length > 3 && (
              <span className="px-2 py-1 bg-surface-alt text-fg rounded-full text-xs">
                +{module.technologies.length - 3} more
              </span>
            )}
          </div>
        ...
        >
          <div className="tier-header mb-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
              <div className="flex-1">
                <div className="flex items-stretch gap-4 mb-4">
                  ...
                  <div className="text-left">
                    <h2
                      className="text-xl md:text-2xl font-bold text-white text-left truncate"
                      id={`tier-${tierKey}-heading`}
                    >
                      {tier.title}
                    </h2>
                    <p className="text-fg mt-1 text-left text-sm">
                      {tier.description}
                    </p>
                  </div>
                </div>
              ...
              <div className="md:flex-none md:w-[500px] w-full grid grid-cols-2 gap-3">
                <div className="glass-morphism p-3 rounded-lg w-full">
                  <h3 className="text-xs font-semibold uppercase tracking-wide text-white/80 text-left mb-1">
                    Focus Area
                  </h3>
            ...
            <div className="mf-desktop-only glass-morphism rounded-xl p-5 mt-4">
              <h3 className="text-lg font-semibold text-fg mb-3 text-left">
                Learning Objectives
              </h3>
          ...
          {/* Modules grid */}
          <div
            className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-0 md:gap-4 mt-6 auto-rows-fr"
            role="list"
            aria-label={`${tier.title} modules`}
          >
              ...
              {tierKey === 'quality' && (
                <div className="mt-4">
                  ...
                  <p className="text-white/90 mt-1">
                    You&apos;ve completed all tiers and mastered full stack
                    development!
                  </p>
                </div>
              )}
...
const HomePage: React.FC<{ initialRegistryData?: RegistryData | null }> = ({
  initialRegistryData,
}) => {
  const [registryData, setRegistryData] = useState<RegistryData | null>(
    initialRegistryData ?? null
  );
  const [loading, setLoading] = useState(!initialRegistryData);
      ...
      if ((normalizedModules?.length || 0) === 0) {
        try {
          ...
          if (staticRes.ok) {
            ...
            const staticTiers: Record<string, Tier> =
  ...
  // Pre-fetch data on component mount (skip if server-provided initial data exists)
  useEffect(() => {
  ...
  }, [loadRegistryData, initialRegistryData]);

  // Memoize filtered tiers to prevent unnecessary re-renders
  const filteredTiers = useMemo(() => {
    if (!registryData) return {};

    return Object.entries(registryData.tiers).reduce(
      (acc, [tierKey, tierData]) => {
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
                <div className="hero-content">
                  <div className="flex items-center justify-between mb-4">
                    <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-white text-left">
                  ...
                  <p className="text-lg md:text-xl text-muted mb-6 text-left">
                    Comprehensive learning paths across 18 technology modules
                    with interactive lessons, real-world projects, and interview
                    preparation.
                  </p>
                  ...
                  {recentAchievements.length > 0 && (
                    <div className="mt-6">
                      <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-3 text-left">
                        Recent Achievements
                      </h2>
                ...
                <div className="hero-visual mf-desktop-only relative flex justify-center">
                  <div className="learning-path-visualization w-full max-w-md">
                    <svg
                      viewBox="0 0 400 300"
                      className="path-svg w-full h-auto"
                      role="img"
                      aria-label="Learning path visualization"
                    >
                      <defs>
                        <linearGradient
                          id="pathGradient"
                      ...
                      <circle
                        ...
                        fill="#10B981"
                        className="tier-node"
                        aria-label="Core technologies tier"
                      />
                      <circle
                        cx="250"
                        cy="110"
                        ...
                        fill="#8B5CF6"
                        className="tier-node"
                        aria-label="Specialised skills tier"
                      />
                      <circle
                        cx="350"
                        cy="50"
                        ...
                        fill="#F59E0B"
                        className="tier-node"
                        aria-label="Quality and testing tier"
                      />
                      <text
                        x="50"
                        y="275"
          ...
          <div className="glass-morphism md:rounded-xl mf-no-radius-mobile">
            <div className="p-6">
              <h2
                ...
                className="text-2xl font-bold text-center text-fg mb-6"
              >
                Quick Actions
              </h2>
              <div
