...
export default function LessonsPage() {
  ...
  const topicGroups: TopicGroup[] = Object.values(
    lessons.reduce(
      (acc, lesson) => {
        if (!acc[lesson.topic])
          acc[lesson.topic] = { topic: lesson.topic, lessons: [] };
  ...
  if (selectedTopic !== null && selectedIndex !== null) {
    currentTopicLessons =
      topicGroups.find((tg) => tg.topic === selectedTopic)?.lessons ?? [];
    ...
    nextCategoryTopic =
      topicGroups[(currentTopicIdx + 1) % topicGroups.length]?.topic ?? null;
  }
  ...
  return (
    <div className="w-full p-6">
      ...
      <div className="max-w-2xl mx-auto bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700">
        <Link
          href="/"
          className="inline-block mb-4 bg-teal-100 dark:bg-teal-900 text-teal-800 dark:text-teal-200 font-semibold py-1 px-2 rounded shadow hover:bg-teal-200 dark:hover:bg-teal-800 transition-colors duration-150 flex items-center gap-1 text-xs"
        >
          <span className="text-base">←</span> Back to Home
        </Link>
        <h1 className="text-3xl font-bold mb-6 mt-2 text-teal-700 dark:text-teal-300">
            ...
            {topicGroups.map((group) => (
              <div key={group.topic} className="mb-10">
                <h2 className="text-2xl font-extrabold mb-4 flex items-center gap-2 text-teal-700 dark:text-teal-300">
                  ...
                  <span className="bg-teal-50 dark:bg-teal-900 px-4 py-2 rounded-lg text-teal-800 dark:text-teal-200 shadow-sm">
                    {group.topic}
                  </span>
                </h2>
          ...
          <div className="bg-gray-100/80 dark:bg-gray-700/80 backdrop-blur-sm p-6 rounded-xl shadow-lg space-y-4 mt-4 border border-gray-200 dark:border-gray-600">
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm text-gray-600 dark:text-gray-400">
                Lesson {currentLessonIndex! + 1} of {currentTopicLessons.length}
              </span>
            </div>
