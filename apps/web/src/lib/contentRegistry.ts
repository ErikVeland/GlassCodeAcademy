...
class ContentRegistryLoader {
  ...
  async loadRegistry(): Promise<ContentRegistry> {
    ...
    this.registryPromise = this.loadRegistryInternal()
      .then((registry) => {
  ...
  async findModuleByLegacySlug(legacySlug: string): Promise<Module | null> {
    const modules = await this.getModules();
    return modules.find((mod) => mod.legacySlugs.includes(legacySlug)) || null;
  }
      ...
      try {
        ...
        if (isBrowser) {
          ...
          const lessonsArr: PartialLesson[] = Array.isArray(data)
            ...
            : Array.isArray((data as { lessons?: PartialLesson[] })?.lessons)
              ? ((data as { lessons?: PartialLesson[] })
  ...
  async checkModuleThresholds(moduleSlug: string): Promise<{
    ...
    try {
      ...
      const lessonsMeetThreshold =
        lessons.length >= (mod.thresholds?.requiredLessons || 0);
      const quizMeetsThreshold =
        !!quiz &&
        (quiz?.questions?.length ?? 0) >=
          (mod.thresholds?.requiredQuestions || 0);
...
export function getLessonGroups(
  ...
  for (const lesson of lessons) {
    const legacyTopic = (
      lesson as unknown as { legacy?: { originalTopic?: string } }
    ).legacy?.originalTopic;
  ...
  if (topics.size > 0) {
    const groups: LessonGroup[] = Array.from(topics.entries()).map(
