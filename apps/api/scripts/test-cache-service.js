...
async function testCacheService() {
  ...
  const setSettingsSuccess = await cacheService.setAcademySettings(academyId, settings, 60);
  ...
  const setPermissionsSuccess = await cacheService.setUserPermissions(userId, academyId, permissions, 60);
  console.log(`Set user permissions successful: ${setPermissionsSuccess}`);
  
  const retrievedPermissions = await cacheService.getUserPermissions(userId, academyId);
  console.log(`Retrieved user permissions:`, retrievedPermissions);
  
  if (retrievedPermissions && retrievedPermissions.length === permissions.length) {
  ...
  const setMembershipSuccess = await cacheService.setMembership(userId, academyId, membership, 60);
  console.log(`Set membership successful: ${setMembershipSuccess}`);
  
  const retrievedMembership = await cacheService.getMembership(userId, academyId);
  ...
  const setCourseSuccess = await cacheService.setCourse(courseId, courseData, 60);
