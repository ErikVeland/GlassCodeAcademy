  ...
  try {
    ...
    const total =
      ...
      (json.testResults || []).reduce(
        (a, tr) => a + tr.assertionResults.length,
        0
      );
    const passed =
      json.numPassedTests ??
      (json.testResults || []).reduce(
        (a, tr) =>
          a + tr.assertionResults.filter((ar) => ar.status === 'passed').length,
        0
      );
    const failed =
      json.numFailedTests ??
      (json.testResults || []).reduce(
        (a, tr) =>
          a + tr.assertionResults.filter((ar) => ar.status === 'failed').length,
        0
      );
