...
async function migrateLessons(modules) {
  ...
  for (const module of modules) {
    try {
      const lessonFilePath = path.join(__dirname, '../../../../content/lessons', `${module.slug}.json`);
      const lessonData = JSON.parse(await fs.readFile(lessonFilePath, 'utf8'));
      
      for (const lesson of lessonData) {
        const lessonRecord = await Lesson.create(convertLesson(lesson, module.id));
        lessonCount++;
        console.log(`Created lesson: ${lessonRecord.title}`);
      }
    } catch (error) {
      console.error(`Error migrating lessons for module ${module.slug}:`, error.message);
    }
  }
...
async function migrateQuizzes(modules) {
  ...
  for (const module of modules) {
    try {
      ...
      const quizFilePath = path.join(__dirname, '../../../../content/quizzes', `${module.slug}.json`);
      ...
      for (const question of quizData.questions) {
        ...
        const lessonId = lessonMap[question.id] || (lessons.length > 0 ? lessons[0].id : null);
        
        if (lessonId) {
          const quizRecord = await Quiz.create(convertQuizQuestion(question, lessonId));
          quizCount++;
          console.log(`Created quiz question: ${quizRecord.id}`);
        }
      }
    } catch (error) {
      console.error(`Error migrating quizzes for module ${module.slug}:`, error.message);
    }
  }
