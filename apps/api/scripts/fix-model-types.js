...
function fixModel(filePath) {
  ...
  if (
    !hasHelper &&
    (content.includes('DataTypes.JSONB') || content.includes('DataTypes.ARRAY'))
  ) {
    const requireLine =
      "const { DataTypes } = require('sequelize');\nconst sequelize = require('../config/database');";
    const newRequireLine =
  ...
  if (jsonbMatches) {
    content = content.replace(
      /type:\s*DataTypes\.JSONB/g,
      'type: getJSONType()'
    );
    console.log(
      `  ✓ Replaced ${jsonbMatches.length} DataTypes.JSONB with getJSONType()`
    );
    modified = true;
  }
  ...
  if (arrayMatches.length > 0) {
    ...
    console.log(
      `  ✓ Replaced ${arrayMatches.length} DataTypes.ARRAY with getArrayType()`
    );
      ...
      if (fieldNameMatch) {
        ...
        if (!fieldDef.includes('get:') && !fieldDef.includes('set:')) {
          ...
          if (content[closingBraceIndex - 1] === ',') {
            content =
              content.substring(0, closingBraceIndex - 1) +
