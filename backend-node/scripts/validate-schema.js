...
async function main() {
  ...
  for (const table of tablesToCheck) {
    ...
    if (columns) {
      ...
      console.log(`\n✓ ${table}:`);
      console.log(
        `  Columns: ${columns.map((col) => col.column_name).join(', ')}`
      );
    } else {
  ...
  const indexesToCreate = [
    {
      ...
      columns: ['user_id', 'academy_id'],
      name: 'idx_membership_user_academy',
    },
    {
      table: 'academy_memberships',
      columns: ['academy_id', 'role_id'],
      name: 'idx_membership_academy_role',
    },
    {
      table: 'academy_settings',
      columns: ['academy_id', 'category'],
      name: 'idx_settings_academy_category',
    },
    {
      table: 'content_workflows',
      columns: ['academy_id', 'content_type'],
      name: 'idx_workflow_academy_status',
    },
    {
      table: 'content_approvals',
      columns: ['academy_id', 'status'],
      name: 'idx_approval_academy_status',
    },
    {
    ...
    },
    {
      ...
      columns: ['academy_id', 'status'],
      name: 'idx_package_academy_status',
    },
    {
      table: 'content_imports',
      columns: ['academy_id', 'status'],
      name: 'idx_import_academy_status',
    },
    {
      table: 'departments',
      columns: ['academy_id', 'parent_id'],
    ...
    {
      table: 'courses',
      columns: ['academy_id', 'is_published'],
      name: 'idx_course_academy_published',
    },
    {
      table: 'modules',
      columns: ['academy_id', 'course_id'],
      name: 'idx_module_academy_course',
    },
    {
      table: 'lessons',
      columns: ['academy_id', 'module_id'],
      name: 'idx_lesson_academy_module',
    },
    {
      table: 'lesson_quizzes',
      columns: ['academy_id', 'lesson_id'],
      name: 'idx_quiz_academy_lesson',
    },
    {
      table: 'user_progress',
      columns: ['user_id', 'lesson_id'],
      name: 'idx_progress_user_lesson',
    },
    {
    ...
    {
      table: 'assets',
      columns: ['academy_id', 'asset_type'],
      name: 'idx_asset_academy_type',
    },
    {
      table: 'asset_usage',
      columns: ['asset_id', 'content_type'],
      name: 'idx_asset_usage_content',
    },
    {
      table: 'validation_rules',
      columns: ['academy_id', 'content_type'],
      name: 'idx_validation_academy_content',
    },
    {
      table: 'validation_results',
      columns: ['validation_rule_id', 'status'],
      name: 'idx_validation_result_status',
    },
  ];
  ...
  validIndexes.forEach((idx) => {
    ...
    console.log(
      `  ${status} ${idx.name} on ${idx.table}(${idx.columns.join(', ')})`
    );
  });
