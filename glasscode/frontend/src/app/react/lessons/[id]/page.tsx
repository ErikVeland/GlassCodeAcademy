...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  const currentModule = await contentRegistry.findModuleByRoutePath('/react/lessons');
  if (!currentModule) {
...
export default async function ReactLessonPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const currentModule = await contentRegistry.findModuleByRoutePath('/react/lessons');
