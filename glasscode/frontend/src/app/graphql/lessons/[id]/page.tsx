...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  const currentModule = await contentRegistry.findModuleByRoutePath('/graphql/lessons');
  if (!currentModule) {
...
export default async function GraphqlLessonPage({ params }: { params: Promise<{ id: string }> }) {
  const resolvedParams = await params;
