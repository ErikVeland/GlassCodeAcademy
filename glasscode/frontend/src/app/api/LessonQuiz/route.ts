...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!Number.isFinite(lessonIdNumeric) || lessonIdNumeric <= 0) {
      return NextResponse.json(
        { error: "lessonId must be a positive number" },
        { status: 400 },
      );
    }
    const apiBase = (() => {
    ...
    const publicRes = await retryFetch(
      publicUrl,
    ...
    const lessonRes = await retryFetch(
      `${apiBase}/api/lessons/${lessonIdNumeric}`,
      { cache: "no-store", headers: { "Content-Type": "application/json" } },
      3,
      250,
    );
    ...
    const moduleRes = await retryFetch(
    ...
    const moduleSlug = isRecord(moduleData)
      ? (getString(moduleData.slug) ??
        getString(moduleData.shortSlug) ??
        getString(moduleData.moduleSlug))
      : undefined;
    ...
    const moduleQuizRes = await retryFetch(
    ...
    return new NextResponse(
      JSON.stringify(Array.isArray(moduleQuizData) ? moduleQuizData : []),
      {
        status: moduleQuizRes.status,
        headers: { "Content-Type": "application/json" },
      },
    );
  } catch (error) {
    ...
    return NextResponse.json(
      { error: "Failed to fetch quizzes from backend" },
      { status: 502 },
    );
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const lessonId = isRecord(rawBody)
      ? getNumber(rawBody.lessonId)
      : undefined;
    if (!lessonId) {
      return NextResponse.json(
    ...
    const res = await fetch(
      `${apiBase}/api/content/lessons/${lessonId}/quizzes`,
      {
        method: "POST",
        headers: {
    ...
    return NextResponse.json(
      { error: "Failed to create quiz in backend" },
      { status: 502 },
    );
  }
}
