...
interface TypescriptInterviewQuestion {
  id: number;
  topic: string;
  type: string;
  question: string;
...
const TYPESCRIPT_QUIZ_STORAGE_KEY = "typescript_quiz_state_v1";

const TYPESCRIPT_INTERVIEW_QUESTIONS_QUERY = gql`
  query TypescriptInterviewQuestions {
...
const SUBMIT_TYPESCRIPT_ANSWER_MUTATION = gql`
  mutation SubmitTypescriptAnswer($questionId: Int!, $answerIndex: Int!) {
    submitTypescriptAnswer(questionId: $questionId, answerIndex: $answerIndex) {
      isCorrect
      explanation
    }
  }
...
function formatQuestionText(text: string) {
  return text.replace(/\b(type|interface|enum|class|function|const|let|var|extends|implements|public|private|protected|static|readonly|abstract|async|await|Promise|Observable|any|unknown|never|void|undefined|null|boolean|number|string|symbol|bigint|object|Array|Tuple|Generic|Union|Intersection|Conditional|Mapped|Infer|Template|Literal|Index|Keyof|Typeof|Instanceof|As|Satisfies|Namespace|Module|Import|Export|Default|Declare|Override|Accessor|Decorator|Experimental|Metadata|Reflect|Proxy|Handler|Target|This|New|Type|Assertion|NonNullable|Partial|Required|Readonly|Record|Pick|Omit|Exclude|Extract|ReturnType|InstanceType|ThisParameterType|OmitThisParameter|Uppercase|Lowercase|Capitalize|Uncapitalize)\b/g, '<code class="bg-gray-100 dark:bg-gray-700 px-1 py-0.5 rounded text-sm font-mono">$1</code>');
}
...
function shuffleQuestionChoices(question: TypescriptInterviewQuestion): TypescriptInterviewQuestion {
...
export default function TypescriptInterviewPage() {
  const [shuffledQuestions, setShuffledQuestions] = useState<TypescriptInterviewQuestion[]>([]);
  ...
  const { data, loading: gqlLoading, error: gqlError, refetch } = useQuery(TYPESCRIPT_INTERVIEW_QUESTIONS_QUERY);
  ...
  const gqlQuestions: TypescriptInterviewQuestion[] = data?.typescriptInterviewQuestions ?? [];

  const [submitAnswer] = useMutation(SUBMIT_TYPESCRIPT_ANSWER_MUTATION);

  useEffect(() => {
    const saved = localStorage.getItem(TYPESCRIPT_QUIZ_STORAGE_KEY);
    if (saved) {
      try {
  ...
  const handleSubmit = async () => {
    ...
    setFeedback(data.submitTypescriptAnswer);
    if (data.submitTypescriptAnswer.isCorrect) setScore((s) => s + 1);
  };
  ...
  const clearQuizState = () => {
    localStorage.removeItem(TYPESCRIPT_QUIZ_STORAGE_KEY);
  };
      ...
      <div className="py-12 px-4 sm:px-6 lg:px-8">
        <div className="bg-white/30 dark:bg-gray-800/30 backdrop-blur-sm rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700">
          <div className="text-center">
            ...
            <p className="mb-4 text-gray-600 dark:text-gray-300">There are no TypeScript interview questions available at this time.</p>
        ...
        <div className="bg-white/30 dark:bg-gray-800/30 backdrop-blur-sm rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700">
          <div className="text-center">
            ...
            <div className="mb-6">
              ...
              <p className="mt-1 text-gray-600 dark:text-gray-300">
                {score >= Math.ceil(shuffledQuestions.length * 0.7) 
                  ? 'Great job! You have successfully passed the TypeScript interview quiz.' 
            ...
            <div className="flex flex-wrap justify-center gap-4 mt-8">
              ...
              {score < Math.ceil(shuffledQuestions.length * 0.7) && (
                <Link href="/typescript/lessons" className="px-4 py-2 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 text-white rounded-lg hover:from-indigo-600 hover:via-purple-600 hover:to-pink-600 transition-all duration-150 font-semibold">
                  Review Lessons
                </Link>
              )}
              <button
            ...
            {score >= Math.ceil(shuffledQuestions.length * 0.7) && (
              <div className="mt-8 p-6 border-2 border-blue-200 dark:border-blue-700 rounded-xl bg-blue-50/30 dark:bg-blue-900/20 backdrop-blur-sm">
                <h3 className="text-xl font-bold text-blue-800 dark:text-blue-200 mb-2">Certificate of Completion</h3>
                <p className="text-blue-700 dark:text-blue-300">This certifies that you have successfully completed the TypeScript interview preparation quiz.</p>
              </div>
            )}
          </div>
        </div>
  ...
  return (
    ...
    <div className="py-12 px-4 sm:px-6 lg:px-8">
                  ...
                  <div 
                    ...
                    className={`p-4 border rounded-lg cursor-pointer transition-all duration-200 backdrop-blur-sm ${
                      feedback 
                        ...
                        : displayIndex === selected 
                          ? "bg-blue-50/30 dark:bg-blue-900/20 border-blue-300 dark:border-blue-600 shadow-sm" 
                          : "border-gray-200 dark:border-gray-600 hover:border-blue-200 dark:hover:border-blue-500 hover:bg-blue-50/30 dark:hover:bg-blue-900/10"
                    }`}
                  >
      ...
      <TechnologyUtilizationBox 
        ...
        explanation="In this TypeScript module, TypeScript is being used as the core frontend framework to build the entire user interface. TypeScript components manage the state and rendering of the lesson content and quiz questions." 
      />
    </div>
  );
}