...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  const currentModule = await contentRegistry.findModuleByRoutePath('/typescript/lessons');
  if (!currentModule) {
...
export default async function TypescriptLessonPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const currentModule = await contentRegistry.findModuleByRoutePath('/typescript/lessons');
