...
interface PerformanceInterviewQuestion {
  id: number;
  topic: string;
  type: string;
  question: string;
...
const PERFORMANCE_QUIZ_STORAGE_KEY = "performance_quiz_state_v1";

const PERFORMANCE_INTERVIEW_QUESTIONS_QUERY = gql`
  query PerformanceInterviewQuestions {
...
const SUBMIT_PERFORMANCE_ANSWER_MUTATION = gql`
  mutation SubmitPerformanceAnswer($questionId: Int!, $answerIndex: Int!) {
    submitPerformanceAnswer(questionId: $questionId, answerIndex: $answerIndex) {
      isCorrect
      explanation
    }
  }
...
function formatQuestionText(text: string) {
  return text.replace(/\b(Profiling|Monitoring|Optimization|Bundle Size|Tree Shaking|Code Splitting|Lazy Loading|Caching|CDN|Minification|Compression|Debouncing|Throttling|Memoization|Virtualization|Web Workers|Service Workers|Progressive Web Apps|Lighthouse|Web Vitals|Core Web Vitals|LCP|FID|CLS|FCP|TTFB)\b/g, '<code class="bg-gray-100 dark:bg-gray-700 px-1 py-0.5 rounded text-sm font-mono">$1</code>');
}
...
function shuffleQuestionChoices(question: PerformanceInterviewQuestion): PerformanceInterviewQuestion {
...
export default function PerformanceInterviewPage() {
  const [shuffledQuestions, setShuffledQuestions] = useState<PerformanceInterviewQuestion[]>([]);
  ...
  const gqlQuestions: PerformanceInterviewQuestion[] = data?.performanceInterviewQuestions ?? [];

  const [submitAnswer] = useMutation(SUBMIT_PERFORMANCE_ANSWER_MUTATION);

  useEffect(() => {
    const saved = localStorage.getItem(PERFORMANCE_QUIZ_STORAGE_KEY);
    if (saved) {
      try {
  ...
  const handleSubmit = async () => {
    ...
    setFeedback(data.submitPerformanceAnswer);
    if (data.submitPerformanceAnswer.isCorrect) setScore((s) => s + 1);
  };
  ...
  const clearQuizState = () => {
    localStorage.removeItem(PERFORMANCE_QUIZ_STORAGE_KEY);
  };
      ...
      <div className="py-12 px-4 sm:px-6 lg:px-8">
        <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700">
          <div className="text-center">
            ...
            <p className="mb-4 text-gray-600 dark:text-gray-300">There are no Performance Optimization interview questions available at this time.</p>
        ...
        <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700">
          <div className="text-center">
            ...
            <div className="mb-6">
              ...
              <p className="mt-1 text-gray-600 dark:text-gray-300">
                {score >= Math.ceil(shuffledQuestions.length * 0.7) 
                  ? 'Great job! You have successfully passed the Performance Optimization interview quiz.' 
            ...
            <div className="flex flex-wrap justify-center gap-4 mt-8">
              ...
              {score < Math.ceil(shuffledQuestions.length * 0.7) && (
                <Link href="/performance/lessons" className="px-4 py-2 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 text-white rounded-lg hover:from-indigo-600 hover:via-purple-600 hover:to-pink-600 transition-all duration-150 font-semibold">
                  Review Lessons
                </Link>
              )}
              <button
            ...
            {score >= Math.ceil(shuffledQuestions.length * 0.7) && (
              <div className="mt-8 p-6 border-2 border-blue-200 dark:border-blue-700 rounded-xl bg-blue-50/80 dark:bg-blue-900/30 backdrop-blur-sm">
                ...
                <p className="text-blue-700 dark:text-blue-300">This certifies that you have successfully completed the Performance Optimization interview preparation quiz.</p>
              </div>
            )}
          </div>
        </div>
  ...
  return (
    ...
    <div className="py-12 px-4 sm:px-6 lg:px-8">
      ...
      <TechnologyUtilizationBox 
        ...
        explanation="In this Performance Optimization module, profiling, monitoring, and optimization techniques are taught using real-world examples." 
      />
    </div>
  );
}