...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  const currentModule = await contentRegistry.findModuleByRoutePath('/nextjs/lessons');
  if (!currentModule) {
...
export default async function NextjsLessonPage({ params }: { params: Promise<{ id: string }> }) {
  const resolvedParams = await params;
