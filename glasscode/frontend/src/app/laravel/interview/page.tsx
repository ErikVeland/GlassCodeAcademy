...
const LARAVEL_QUIZ_STORAGE_KEY = "laravel_quiz_state_v1";
...
export default function LaravelInterviewPage() {
  const [shuffledQuestions, setShuffledQuestions] = useState<LaravelInterviewQuestion[]>([]);
  ...
  const [submitAnswer] = useMutation(SUBMIT_LARAVEL_ANSWER_MUTATION);

  useEffect(() => {
    const saved = localStorage.getItem(LARAVEL_QUIZ_STORAGE_KEY);
    if (saved) {
      try {
  ...
  const handleSubmit = async () => {
    ...
    setFeedback(data.submitLaravelAnswer);
    if (data.submitLaravelAnswer.isCorrect) setScore((s) => s + 1);
  };
  ...
  const clearQuizState = () => {
    localStorage.removeItem(LARAVEL_QUIZ_STORAGE_KEY);
  };
        ...
        <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700">
          <div className="text-center">
            ...
            <div className="flex flex-wrap justify-center gap-4 mt-8">
              ...
              {score < Math.ceil(shuffledQuestions.length * 0.7) && (
                <Link href="/laravel/lessons" className="px-4 py-2 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 text-white rounded-lg hover:from-indigo-600 hover:via-purple-600 hover:to-pink-600 transition-all duration-150 font-semibold">
                  Review Lessons
                </Link>
              )}
              <button
        ...
        <div className="p-6">
          ...
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <h3 className="text-lg font-medium text-red-600 dark:text-red-400">{q.topic}</h3>
            </div>
