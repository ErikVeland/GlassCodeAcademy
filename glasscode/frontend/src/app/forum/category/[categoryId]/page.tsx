...
async function fetchThreads(categoryId: string): Promise<{ threads: Thread[]; pagination?: Pagination }> {
  ...
  const json = await res.json() as { success: boolean; data: Thread[]; pagination?: Pagination };
    ...
    <div className="liquid-glass-layout min-h-screen bg-bg">
      <div className="liquid-glass-content mx-auto py-8">
        <div className="mb-6 flex items-center justify-between">
          ...
          <Link href="/forum" className="text-sm px-3 py-2 bg-surface-alt border border-border rounded-md hover:opacity-90">← Back to Categories</Link>
        </div>
              ...
              <div className="glass-card p-6 hover:opacity-95 transition">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    {t.isPinned && <span className="text-xs px-2 py-1 rounded-full bg-yellow-500/20 text-yellow-300 border border-yellow-400/30">Pinned</span>}
                    {t.isLocked && <span className="text-xs px-2 py-1 rounded-full bg-danger/20 text-danger border border-danger/30">Locked</span>}
                  </div>
                </div>
                <div className="mt-2 text-sm text-muted">
                  <span>Replies: {t.replyCount ?? 0}</span>
                  {t.lastReplyAt && (
                    <span className="ml-3">Last reply: {new Date(t.lastReplyAt).toLocaleString()}</span>
                  )}
                  {t.author?.name && (
