...
interface TestingInterviewQuestion {
  id: number;
  topic: string;
  type: string;
  question: string;
...
const TESTING_QUIZ_STORAGE_KEY = "testing_quiz_state_v1";

const TESTING_INTERVIEW_QUESTIONS_QUERY = gql`
  query TestingInterviewQuestions {
...
const SUBMIT_TESTING_ANSWER_MUTATION = gql`
  mutation SubmitTestingAnswer($questionId: Int!, $answerIndex: Int!) {
    submitTestingAnswer(questionId: $questionId, answerIndex: $answerIndex) {
      isCorrect
      explanation
    }
  }
...
function formatQuestionText(text: string) {
  return text.replace(/\b(Unit Test|Integration Test|End-to-End Test|TDD|BDD|Test Pyramid|Jest|Cypress|Playwright|Selenium|TestCafe|Puppeteer|WebDriverIO|Protractor|Karma|Mocha|Chai|Sinon|Jasmine|QUnit|Ava|Tape|Vitest|React Testing Library|Vue Test Utils|Enzyme|DOM Testing Library|User Event|Test Runner|Test Suite|Test Case|Assertion|Mock|Spy|Stub|Fake|Harness|Fixture|Setup|Teardown|Before All|After All|Before Each|After Each|Describe|It|Test|Expect|Should|Assert|Coverage|Code Coverage|Branch Coverage|Line Coverage|Statement Coverage|Function Coverage|Mutation Testing|Snapshot Testing|Visual Regression Testing|Accessibility Testing|Performance Testing|Load Testing|Stress Testing|Soak Testing|Spike Testing|Volume Testing|Security Testing|Penetration Testing|Fuzz Testing|Compliance Testing|Compatibility Testing|Cross-Browser Testing|Device Testing|Usability Testing|Exploratory Testing|Ad-hoc Testing|Smoke Testing|Sanity Testing|Regression Testing|Re-testing|Positive Testing|Negative Testing|Boundary Value Analysis|Equivalence Partitioning|Decision Table Testing|State Transition Testing|Error Guessing|Use Case Testing|API Testing|Contract Testing|UI Testing|Manual Testing|Automated Testing|Test Plan|Test Strategy|Test Case Design|Test Execution|Test Closure|Defect|Bug|Issue|Ticket|Jira|Bugzilla|TestRail|Zephyr|TestLink|Quality Assurance|Quality Control|Verification|Validation|V Model|W Model|H Model|Agile Testing|Scrum Testing|Kanban Testing|DevOps Testing|Shift-Left Testing|Shift-Right Testing|Continuous Testing|Test Environment|Test Data|Test Automation Framework|Page Object Model|Data-Driven Testing|Keyword-Driven Testing|Hybrid Testing|Linear Scripting|Modular Testing|Test Harness|Test Orchestration|Test Reporting|Test Metrics|Test Dashboard|Test Management|Test Orchestration|CI\/CD|Pipeline|Deployment|Release|Hotfix|Rollback|Rollout|Canary Release|Blue-Green Deployment|Feature Toggle|Feature Flag|A\/B Testing|Multivariate Testing|Chaos Engineering|Chaos Monkey|Gremlin|Litmus|Resilience Testing|Disaster Recovery Testing|Backup Testing|Recovery Testing|Business Continuity Testing|Compliance Testing|Regulatory Testing|SOX|HIPAA|PCI DSS|GDPR|ISO 27001|SOC 2|Audit Trail|Traceability|Requirement Traceability Matrix|RTM|Test Matrix|Risk-Based Testing|Risk Assessment|Risk Mitigation|Test Prioritization|Test Optimization|Test Efficiency|Test Effectiveness|Test Productivity|Test Quality|Test Maturity|Test Governance|Test Policy|Test Standard|Test Process|Test Procedure|Test Script|Test Tool|Test Infrastructure|Test Environment Management|Test Data Management|Test Configuration Management|Test Release Management|Test Incident Management|Test Problem Management|Test Change Management|Test Asset Management|Test Knowledge Management|Test Community|Test Center of Excellence|TCoE|Test Governance|Test Quality|Test Assurance|Test Excellence|Test Innovation|Test Transformation|Test Modernization|Test Digitalization|Test Automation|Test Orchestration|Test Intelligence|Test Analytics|Test AI|Test Machine Learning|Test NLP|Test Computer Vision|Test Robotics|Test IoT|Test Blockchain|Test Cloud|Test Microservices|Test API|Test Web|Test Mobile|Test Desktop|Test Embedded|Test Hardware|Test Firmware|Test Software|Test Application|Test System|Test Solution|Test Service|Test Product|Test Project|Test Program|Test Portfolio|Test Organization|Test Team|Test Role|Test Skill|Test Competency|Test Certification|Test Training|Test Education|Test Learning|Test Development|Test Career|Test Professional|Test Expert|Test Master|Test Guru|Test Wizard|Test Ninja|Test Rockstar|Test Superstar|Test Legend|Test Hero|Test Champion|Test Ambassador|Test Evangelist|Test Advocate|Test Influencer|Test Leader|Test Manager|Test Director|Test VP|Test CTO|Test CIO|Test Architect|Test Designer|Test Analyst|Test Engineer|Test Developer|Test Automation Engineer|Test Performance Engineer|Test Security Engineer|Test DevOps Engineer|Test Cloud Engineer|Test Data Engineer|Test AI Engineer|Test Quality Engineer|Test Assurance Engineer|Test Excellence Engineer|Test Innovation Engineer|Test Transformation Engineer|Test Modernization Engineer|Test Digitalization Engineer)\b/g, '<code class="bg-gray-100 dark:bg-gray-700 px-1 py-0.5 rounded text-sm font-mono">$1</code>');
}
...
function shuffleQuestionChoices(question: TestingInterviewQuestion): TestingInterviewQuestion {
...
export default function TestingInterviewPage() {
  const [shuffledQuestions, setShuffledQuestions] = useState<TestingInterviewQuestion[]>([]);
  ...
  const { data, loading: gqlLoading, error: gqlError, refetch } = useQuery(TESTING_INTERVIEW_QUESTIONS_QUERY);
  ...
  const gqlQuestions: TestingInterviewQuestion[] = data?.testingInterviewQuestions ?? [];

  const [submitAnswer] = useMutation(SUBMIT_TESTING_ANSWER_MUTATION);

  useEffect(() => {
    const saved = localStorage.getItem(TESTING_QUIZ_STORAGE_KEY);
    if (saved) {
      try {
  ...
  const handleSubmit = async () => {
    ...
    setFeedback(data.submitTestingAnswer);
    if (data.submitTestingAnswer.isCorrect) setScore((s) => s + 1);
  };
  ...
  const clearQuizState = () => {
    localStorage.removeItem(TESTING_QUIZ_STORAGE_KEY);
  };
        ...
        <div className="bg-white/30 dark:bg-gray-800/30 backdrop-blur-sm rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700">
          <div className="text-center">
            ...
            <div className="mb-6">
              ...
              <p className="mt-1 text-gray-600 dark:text-gray-300">
                {score >= Math.ceil(shuffledQuestions.length * 0.7) 
                  ? 'Great job! You have successfully passed the Testing interview quiz.' 
            ...
            <div className="flex flex-wrap justify-center gap-4 mt-8">
              ...
              <button
                onClick={restartQuiz}
                className="px-4 py-2 bg-orange-600 dark:bg-orange-700 text-white rounded-lg hover:bg-orange-700 dark:hover:bg-orange-600 transition-colors duration-200"
              >
                Try Again
              </button>
            </div>
        ...
        <div className="p-6">
          ...
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <h3 className="text-lg font-medium text-orange-600 dark:text-orange-400">{q.topic}</h3>
            </div>
