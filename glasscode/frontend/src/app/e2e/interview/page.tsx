...
interface E2eInterviewQuestion {
  id: number;
  topic: string;
  type: string;
  question: string;
...
const E2E_QUIZ_STORAGE_KEY = "e2e_quiz_state_v1";

const E2E_INTERVIEW_QUESTIONS_QUERY = gql`
  query E2eInterviewQuestions {
    e2eInterviewQuestions {
      id
      topic
      type
      question
...
const SUBMIT_E2E_ANSWER_MUTATION = gql`
  mutation SubmitE2eAnswer($questionId: Int!, $answerIndex: Int!) {
    submitE2eAnswer(questionId: $questionId, answerIndex: $answerIndex) {
      isCorrect
      explanation
    }
  }
...
function formatQuestionText(text: string) {
  return text.replace(/\b(Cypress|Playwright|Selenium|Puppeteer|Test Automation|End-to-End Testing|Integration Testing|UI Testing|Assertion|Selector|Command|Hook|Plugin|Reporter|Dashboard|Parallel Testing|Cross-Browser Testing|Headless Mode|Debugging|Screenshots|Videos)\b/g, '<code class="bg-gray-100 dark:bg-gray-700 px-1 py-0.5 rounded text-sm font-mono">$1</code>');
}
...
function shuffleQuestionChoices(question: E2eInterviewQuestion): E2eInterviewQuestion {
...
export default function E2eInterviewPage() {
  const [shuffledQuestions, setShuffledQuestions] = useState<E2eInterviewQuestion[]>([]);
  ...
  const [submitAnswer] = useMutation(SUBMIT_E2E_ANSWER_MUTATION);

  useEffect(() => {
    const saved = localStorage.getItem(E2E_QUIZ_STORAGE_KEY);
    if (saved) {
      try {
  ...
  const handleSubmit = async () => {
    ...
    setFeedback(data.submitE2eAnswer);
    if (data.submitE2eAnswer.isCorrect) setScore((s) => s + 1);
  };
  ...
  const clearQuizState = () => {
    localStorage.removeItem(E2E_QUIZ_STORAGE_KEY);
  };
      ...
      <div className="py-12 px-4 sm:px-6 lg:px-8">
        <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700">
          <div className="text-center">
            ...
            <p className="mb-4 text-gray-600 dark:text-gray-300">There are no E2E Testing interview questions available at this time.</p>
        ...
        <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700">
          <div className="text-center">
            ...
            <div className="mb-6">
              ...
              <p className="mt-1 text-gray-600 dark:text-gray-300">
                {score >= Math.ceil(shuffledQuestions.length * 0.7) 
                  ? 'Great job! You have successfully passed the E2E Testing interview quiz.' 
            ...
            <div className="flex flex-wrap justify-center gap-4 mt-8">
              ...
              {score < Math.ceil(shuffledQuestions.length * 0.7) && (
                <Link href="/e2e/lessons" className="px-4 py-2 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 text-white rounded-lg hover:from-indigo-600 hover:via-purple-600 hover:to-pink-600 transition-all duration-150 font-semibold">
                  Review Lessons
                </Link>
              )}
              <button
            ...
            {score >= Math.ceil(shuffledQuestions.length * 0.7) && (
              <div className="mt-8 p-6 border-2 border-blue-200 dark:border-blue-700 rounded-xl bg-blue-50/80 dark:bg-blue-900/30 backdrop-blur-sm">
                ...
                <p className="text-blue-700 dark:text-blue-300">This certifies that you have successfully completed the E2E Testing interview preparation quiz.</p>
              </div>
            )}
          </div>
        </div>
  ...
  return (
    ...
    <div className="py-12 px-4 sm:px-6 lg:px-8">
      ...
      <TechnologyUtilizationBox 
        technology="E2E Testing" 
        explanation="In this E2E Testing module, Cypress and Playwright frameworks are taught using real-world testing scenarios." 
      />
    </div>
  );
}