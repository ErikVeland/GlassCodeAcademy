...
interface VersionInterviewQuestion {
  id: number;
  topic: string;
  type: string;
  question: string;
...
const VERSION_QUIZ_STORAGE_KEY = "version_quiz_state_v1";

const VERSION_INTERVIEW_QUESTIONS_QUERY = gql`
  query VersionInterviewQuestions {
...
const SUBMIT_VERSION_ANSWER_MUTATION = gql`
  mutation SubmitVersionAnswer($questionId: Int!, $answerIndex: Int!) {
    submitVersionAnswer(questionId: $questionId, answerIndex: $answerIndex) {
      isCorrect
      explanation
    }
  }
...
function formatQuestionText(text: string) {
  return text.replace(/\b(Git|GitHub|Branch|Merge|Commit|Push|Pull|Clone|Fork|Pull Request|Rebase|Stash|Tag|Remote|Origin|Upstream|HEAD|Master|Main|Feature Branch|Hotfix|Release)\b/g, '<code class="bg-gray-100 dark:bg-gray-700 px-1 py-0.5 rounded text-sm font-mono">$1</code>');
}
...
export default function VersionInterviewPage() {
  const [shuffledQuestions, setShuffledQuestions] = useState<VersionInterviewQuestion[]>([]);
  ...
  const gqlQuestions: VersionInterviewQuestion[] = data?.versionInterviewQuestions ?? [];

  const [submitAnswer] = useMutation(SUBMIT_VERSION_ANSWER_MUTATION);

  useEffect(() => {
    const saved = localStorage.getItem(VERSION_QUIZ_STORAGE_KEY);
    if (saved) {
      try {
        const parsed = JSON.parse(saved);
        const savedQuestions: VersionInterviewQuestion[] = Array.isArray(parsed.questions)
          ? (parsed.questions as VersionInterviewQuestion[])
          : [];
        const sanitizedSaved = savedQuestions
          .map((q: VersionInterviewQuestion) => sanitizeQuestion(q))
  ...
  const handleSubmit = async () => {
    ...
    setFeedback(data.submitVersionAnswer);
    if (data.submitVersionAnswer.isCorrect) setScore((s) => s + 1);
  };
  ...
  const clearQuizState = () => {
    localStorage.removeItem(VERSION_QUIZ_STORAGE_KEY);
  };
      ...
      <div className="py-12 px-4 sm:px-6 lg:px-8">
        <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700">
          <div className="text-center">
            ...
            <p className="mb-4 text-gray-600 dark:text-gray-300">There are no Version Control interview questions available at this time.</p>
        ...
        <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700">
          <div className="text-center">
            ...
            <div className="mb-6">
              ...
              <p className="mt-1 text-gray-600 dark:text-gray-300">
                {score >= Math.ceil(shuffledQuestions.length * 0.7) 
                  ? 'Great job! You have successfully passed the Version Control interview quiz.' 
            ...
            <div className="flex flex-wrap justify-center gap-4 mt-8">
              ...
              {score < Math.ceil(shuffledQuestions.length * 0.7) && (
                <Link href="/version/lessons" className="px-4 py-2 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 text-white rounded-lg hover:from-indigo-600 hover:via-purple-600 hover:to-pink-600 transition-all duration-150 font-semibold">
                  Review Lessons
                </Link>
              )}
              <button
            ...
            {score >= Math.ceil(shuffledQuestions.length * 0.7) && (
              <div className="mt-8 p-6 border-2 border-blue-200 dark:border-blue-700 rounded-xl bg-blue-50/80 dark:bg-blue-900/30 backdrop-blur-sm">
                ...
                <p className="text-blue-700 dark:text-blue-300">This certifies that you have successfully completed the Version Control interview preparation quiz.</p>
              </div>
            )}
          </div>
        </div>
  ...
  return (
    ...
    <div className="py-12 px-4 sm:px-6 lg:px-8">
      ...
      <TechnologyUtilizationBox 
        ...
        explanation="In this Version Control module, Git workflows, branching, merging, and collaboration are taught using practical examples." 
      />
    </div>
  );
}