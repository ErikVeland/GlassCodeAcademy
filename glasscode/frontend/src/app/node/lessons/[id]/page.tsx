...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  const currentModule = await contentRegistry.findModuleByRoutePath('/node/lessons');
  if (!currentModule) {
...
export default async function NodeLessonPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const currentModule = await contentRegistry.findModuleByRoutePath('/node/lessons');
