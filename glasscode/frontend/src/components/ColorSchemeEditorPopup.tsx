...
const ColorSchemeEditorPopup: React.FC<ColorSchemeEditorPopupProps> = ({
  ...
  const updatePosition = useCallback((index: number, newPosition: number) => {
    setColorStops(prev => {
      const updated = [...prev];
      updated[index] = { ...updated[index], position: Math.max(0, Math.min(100, newPosition)) };
  ...
  const addColorStop = useCallback(() => {
    setColorStops(prev => {
      const newPosition = prev.length > 0 ? Math.max(...prev.map(s => s.position)) + 10 : 0;
      const updated = [...prev, {
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[80vh] overflow-hidden">
        ...
        <div className="flex items-center justify-between p-6 border-b border-white/10">
          <h2 className="text-xl font-semibold text-white">Color Scheme Editor</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(80vh-120px)]">
                ...
                <div key={index} className="flex items-center gap-4 p-4 bg-white/5 rounded-lg border border-white/10">
                  ...
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      onChange={(e) => updateColor(index, createRgbaFromHex(e.target.value))}
                      className="w-10 h-10 rounded-lg border border-white/20 bg-transparent cursor-pointer"
                    />
                    <div className="flex flex-col">
                      <span className="text-xs text-white/60">Color {index + 1}</span>
                      <span className="text-xs text-white/40 font-mono">{stop.color}</span>
                    </div>
                  </div>
          ...
          {/* Gradient Controls */}
          <div className="space-y-4">
            <h3 className="text-sm font-medium text-white/80">Gradient Controls</h3>
