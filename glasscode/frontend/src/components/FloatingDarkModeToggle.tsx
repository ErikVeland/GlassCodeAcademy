...
export default function FloatingDarkModeToggle() {
  ...
  // Ensure the prehydration click handler is removed after this component mounts
  useEffect(() => {
    try {
    ...
    >
      {theme === "system" ? (
        ...
        <span className="inline-flex items-center justify-center leading-none align-middle">
          <DevicePhoneMobileIcon
            className="h-6 w-6 block sm:hidden shrink-0"
            aria-hidden
          />
          <ComputerDesktopIcon
            className="h-6 w-6 hidden sm:block shrink-0"
            aria-hidden
          />
        </span>
      ) : isDark ? (
